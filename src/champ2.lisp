...
(defmethod format-link ((a fi-info))
    ...
    (let* ((fname (concatenate 'string (pathname-name (pathname (name a))) "." (pathname-type (pathname (name a)))))
        ...
        (format nil "(<a href=\"file://~A\" target=\"new\">~A</a>) - <a href=\"file://~A\" target=\"new\">~A</a>"
            fdir fdir (name a) fname)))
...
(defun main(&rest others2) ;have to fill outputfilename 
    "Main entry point"
    (if (eql (length others2) 0)
    	(champ-usage)
    	(progn
    	    (setq outputfilename (car others2))
    	    (setq others (cdr others2))))
